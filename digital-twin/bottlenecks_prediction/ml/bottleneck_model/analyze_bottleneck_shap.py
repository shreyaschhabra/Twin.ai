...
def main() -> int:
    ...
    margin = base_values + shap_values.sum(axis=1)
    reconstructed_probability = sigmoid(margin)
    model_probability = model.predict_proba(X)[:, 1]
    max_additivity_error = float(np.max(np.abs(reconstructed_probability - model_probability)))
