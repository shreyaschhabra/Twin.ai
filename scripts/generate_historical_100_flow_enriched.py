"""
Step 5 continuation (Decision 36): generate Dataset B, the coverage-
balanced Flow-enrichment 100-shift corpus. Same frozen factory, same
scenario physics, same QC mapping, same master seed (20240002) as
Dataset A (data/generated/historical_100/) -- the ONLY difference is the
scenario SCHEDULING POLICY (backend/historical/flow_enrichment.py), which
adds deterministic, pre-declared Flow-stress opportunities on top of the
unchanged background scheduler. Dataset A is left untouched.

Pipeline boundary enforced by execution order: the enrichment plan is
built and written to disk BEFORE any simulation runs (see
flow_enriched_schedule.json in the output directory).

Usage:
    python scripts/generate_historical_100_flow_enriched.py
"""
...
OUT_BASE = Path(__file__).resolve().parent.parent / "data" / "generated" / "historical_100_flow_enriched"
OBSERVABLE_DIR = OUT_BASE / "observable"
LATENT_DIR = OUT_BASE / "latent"
SCHEDULE_PLAN_PATH = OUT_BASE / "flow_enriched_schedule.json"
MANIFEST_PATH = OUT_BASE / "manifest.json"

N_SHIFTS = 100
DATASET_MASTER_SEED = 20240002  # same master seed as Dataset A; not re-chosen
...
def main():
    ...
    # ---- Step 1: build + freeze the enrichment schedule plan BEFORE any simulation ----
    ...
    # ---- Step 2: generate from the frozen plan ----
    ...
    manifest = {
        "dataset_role": "Dataset B -- scenario-enriched Flow modeling corpus (Decision 36). "
                        "NOT a claim about production scenario frequency; see Dataset A "
                        "(data/generated/historical_100/) for the naturalistic comparison.",
        ...
        "output_stats": stats,
        "note": "The enrichment plan (flow_enriched_schedule.json) was built and written to "
                "disk BEFORE this simulation ran -- see backend/historical/flow_enrichment.py "
                "and tests/test_flow_enrichment_schedule.py for the no-outcome-dependency proof.",
    }
    MANIFEST_PATH.parent.mkdir(parents=True, exist_ok=True)
    with MANIFEST_PATH.open("w") as f:
        json.dump(manifest, f, indent=2)

    print(f"Generated {N_SHIFTS} shifts, {len(qc_df)} vehicles in {generation_seconds:.1f}s (streaming, enriched schedule)")
